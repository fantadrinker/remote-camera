...
const recordAndSave = () => {
    ...
    intervalId.value = window.setInterval(async () => {
        ...
        try {
            ...
            await recordAndUpload(stream, recordingLength, token, user.value.sub || "default");
        } catch (e) {
    ...
    <div class="flex flex-col justify-evenly pt-3 sm:mt-3 w-full sm:w-96 absolute sm:relative bottom-0 bg-zinc-800 sm:bg-transparent bg-opacity-90 rounded-ss-2xl rounded-se-2xl">
        ...
        <div v-show="expandedBottomBar" class="recording-options" v-if="camVideo.isCamOpen" >
            ...
            <template v-if="expandableState.recording">
                ...
                <button class="mx-4 my-2" v-if="isRecording" @click="stopRecording">Stop Recording</button>
                <button class="mx-4 my-2" v-else @click="recordAndSave">Start Recording</button>
            </template>
        </div>
        ...
        <div v-show="expandedBottomBar" class="broadcasting-options" v-if="camVideo.isCamOpen">
            <div class="flex flex-col justify-between">
                ...
                <button @click="toggleBroadcastingOptions">{{ expandableState.broadcast? '-': '|' }}</button>
            </div>
